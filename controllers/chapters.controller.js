import Chapters from '../models/chapters.model.js';
...
//   }
  ...
  try {
    const newChapters = new Chapters({
      ...
      const saveChapters = await newChapters.save();
      res.status(201).json(saveChapters);
  } catch (error) {
    console.log(error);
  }
...
//       }),
//     }
//     const posts = await Post.find(filter,{content : 0})
...
export const gettitlechapter = async (req, res, next) => {
  try {
    const posts =await Chapters.find({
      ...(req.query.slug && { slug: req.query.slug }),
  }, {content : 0})
...
export const getchapter = async (req, res, next) => {
  ...
  try {
    const posts =await Chapters.findOne({
      slug : req.query.slug,
      chaptersNumber: req.query.chaptersNumber
      // ...(req.query.slug && { $or : [
      //   {slug : req.query.slug},
      //   {chaptersNumber : req.query.chaptersNumber}
      // ] }),
  ...
  const decompressedText = decompressedBuffer.toString('utf8');
   ...
   posts.content = decompressedText;
